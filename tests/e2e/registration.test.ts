...
test.describe('User Registration', () => {
  test('should successfully register a new user with valid data', async ({ api, testUser }) => {
  ...
  test('should reject registration with missing required fields', async ({ api }) => {
    const testCases = [
      {
        name: 'Missing name',
        data: { email: 'test@example.com', company_name: 'Test Company', password: 'password123' },
      },
      {
        name: 'Missing email',
        data: { name: 'Test User', company_name: 'Test Company', password: 'password123' },
      },
      {
        name: 'Missing password',
        data: { name: 'Test User', email: 'test@example.com', company_name: 'Test Company' },
      },
      {
    ...
    for (const testCase of testCases) {
      ...
      expect(data.error).toBe('Name, email, password, and company_name are required');
    }
  });

  test('should accept various password lengths during registration', async ({ api }) => {
    const testPasswords = [
      'short', // 5 chars
      'password', // 8 chars
  ...
  test('should return proper response structure on successful registration', async ({ api }) => {
